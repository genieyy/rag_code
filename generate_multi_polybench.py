...
def main(args):
    ...
    results_dir = "./polybench/results_rollback_timefeedback_all"
    os.makedirs(results_dir, exist_ok=True)


    if args.use_rag:
        polybench_record_path = "./polybench/results_rollback_timefeedback_all/multigenerate_result.jsonl"
        opt_dir="./polybench"
    else:
        polybench_record_path = "./polybench/results_rollback_timefeedback_all/multigenerate_result_norag.jsonl"
        opt_dir="./polybench/polybench_norag"
        os.makedirs(opt_dir, exist_ok=True)
    logger = log.setup_logging(record_path=polybench_record_path)  # 获取一个logger对象
    ################################################
    ...
    please provide {args.method_num} best optimized codes.Do not define a new function. The original variable does not need to be redefined. If you generate a new variable, please use the double type.
    Do not output the original C program in your response:
    {search_code}
    there are some examples maybe helpful(if not, ignore it):
        """
    ...
    def code_after_time_feedback(
        ...
        opt_file = deal_data.code_recombine_polybench(
            opt_code,
            basename,
            idx="final",
        ...
        error = deal_data.polybench_compile(
            file_path=file_path_,
            header_path=os.path.dirname(file_path_),
    ...
    def data_dealings(
        ...
        if error is None:
            ...
            a[basename_idx], check[basename_idx], run[basename_idx] = (
                deal_data.polybench_checksum_time_test_exec(exec_path + ".out", ori_sum)
            )
            if check[basename_idx] == False:
                elemcheck[basename_idx] = False
                run[basename_idx] = float("inf")
            else:
                if deal_data.polybench_elemwise(
            ...
            with open("./polybench/compile_result/error.txt", "a") as f:
                f.write(
            ...
            if error is None:
                ...
                run[basename_idx] = float("inf")
                with open(
                    "./polybench/compile_result/error.txt", "a"
                ) as f:
                    f.write("\n" + error)
    ...
    with open("./data/raw_data/rag_polybench_30_1010.json", "r") as searchcodes:
        ...
        for i, data in enumerate(searchcodes):
            ...
            if args.use_rag:
                ...
                if n == args.search_size:
                    ...
                    prompt = first_prompt(search_code, args.use_rag, results)
            ...
            ###################extract opt_code######################################
            opt_codes = extract_code_and_text(response)
            while len(opt_codes) != args.method_num:
                if len(opt_codes) > args.method_num:
                    opt_codes = opt_codes[-args.method_num:]
                elif len(opt_codes) < args.method_num:
                    new_num = args.method_num - len(opt_codes)
                    new_messages = (
                        messages
                        ...
                        + [
                            {
                                "role": "user",
                                "content": f"You did'nt output completely.Please provide {new_num} more code blocks.",
                            }
                        ]
                    )
                    ...
                    response += llm_api[args.llm](messages)
                    print("llm_response:\n", response)
                    opt_codes = opt_codes + extract_code_and_text(response)[-new_num:]
            ...
            for idx, opt_code in enumerate(opt_codes):
                ...
                opt_file = deal_data.code_recombine_polybench(
                    ...
                    org_folder_path=ori_dir,
                    opt_folder_path=opt_dir,
                )  ########返回opt_dir/polybench/medley/floyd-warshall/floyd-warshall_0.after.c
