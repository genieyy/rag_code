...
for (int iter = 0; iter < ITERATIONS; iter++){
    for (i = 3; i < nz - 3; i++)
    {
        for (j = 1; j < mz - 1; j++)
        {
            for (k = 1; k < q - 1; k++)
            {
                double dssp_factor = dssp * (5.0 * u[i][j][k][0] - 4.0 * u[i + 1][j][k][0] + u[i + 2][j][k][0]);
                rhs[i][j][k][0] -= dssp_factor;
                for (m = 1; m < p; m++)
                {
                    dssp_factor = dssp * (5.0 * u[i][j][k][m] - 4.0 * u[i + 1][j][k][m] + u[i + 2][j][k][m]);
                    rhs[i][j][k][m] -= dssp_factor;
                }
            }
        }
    }
