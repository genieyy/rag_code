...
for (int iter = 0; iter < ITERATIONS; iter++){
    for (int jj = 0; jj < m_; jj += 32) {
        for (int kk = 0; kk < q_; kk += 32) {
            for (int mm = 0; mm < p_; mm += 32) {
                for (j = jj; j < min(jj + 32, m_); j++) {
                    for (k = kk; k < min(kk + 32, q_); k++) {
                        for (m = mm; m < min(mm + 32, p_); m++) {
                            forcing[1][j][k][m] = forcing[1][j][k][m] - dssp * (5.0 * ue[1][m] - 4.0 * ue[1 + 1][m] + ue[1 + 2][m]);
                            forcing[2][j][k][m] = forcing[2][j][k][m] - dssp * (-4.0 * ue[2 - 1][m] + 6.0 * ue[2][m] - 4.0 * ue[2 + 1][m] + ue[2 + 2][m]);
                            for (i = 1 * 3; i <= n_ - 3 * 1 - 1; i++) {
                                forcing[i][j][k][m] = forcing[i][j][k][m] - dssp * (ue[i - 2][m] - 4.0 * ue[i - 1][m] + 6.0 * ue[i][m] - 4.0 * ue[i + 1][m] + ue[i + 2][m]);
                            }
                            forcing[n_ - 3][j][k][m] = forcing[n_ - 3][j][k][m] - dssp * (ue[n_ - 3 - 2][m] - 4.0 * ue[n_ - 3 - 1][m] + 6.0 * ue[N - 3][m] - 4.0 * ue[n_ - 3 + 1][m]);
                            forcing[n_ - 2][j][k][m] = forcing[n_ - 2][j][k][m] - dssp * (ue[n_ - 2 - 2][m] - 4.0 * ue[n_ - 2 - 1][m] + 5.0 * ue[n_ - 2][m]);
                        }
                    }
                }
            }
