...
for (int iter = 0; iter < ITERATIONS; iter++){
    for (int i = 0; i < nz; i += 8) {
        for (int j = 0; j < mz; j += 8) {
            for (int k = 0; k < q; k += 8) {
                for (int m = 0; m < p; m += 8) {
                    for (int ii = i; ii < i + 8 && ii < nz; ii++) {
                        xi = (double)ii * dnxm1;

                        for (int jj = j; jj < j + 8 && jj < mz; jj++) {
                            eta = (double)jj * dnym1;

                            for (int kk = k; kk < k + 8 && kk < q; kk++) {
                                zeta = (double)kk * dnzm1;

                                for (int mm = m; mm < m + 8 && mm < p; mm++) {
                                    double Pxi = xi * Pface[1][0][mm] + (1.0 - xi) * Pface[0][0][mm];
                                    double Peta = eta * Pface[1][1][mm] + (1.0 - eta) * Pface[0][1][mm];
                                    double Pzeta = zeta * Pface[1][2][mm] + (1.0 - zeta) * Pface[0][2][mm];
                                    u[ii][jj][kk][mm] = Pxi + Peta + Pzeta - Pxi * Peta - Pxi * Pzeta - Peta * Pzeta + Pxi * Peta * Pzeta;
                                }
                            }
                        }
                    }
