...
for (int iter = 0; iter < ITERATIONS; iter++){
	for (i = ist; i <= iend; i++)
	{
		for (j = jst; j <= jend; j++)
		{
			// Handle the first iteration separately to avoid bounds checking
			if (nz > 1) {
			...
			// Loop unrolling and reduction in division operations
			for (k = 2; k <= nz - 1; k += 2)
			{
			...
			// Handle the last iteration if nz is odd
			if ((nz - 1) % 2 == 1) {
				int k = nz - 1;
				double tmp1 = 1.0 / rsd[i][j][k][0];
				double tmp2 = 1.0 / rsd[i][j][k - 1][0];
