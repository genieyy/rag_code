...
for (int iter = 0; iter < ITERATIONS; iter++){
  for (i = ist; i <= iend; i++) {
    for (j = jst; j <= jend; j++) {
      for (k = 1; k < nz - 1; k++) {
        double temp0 = omega * ldz[i][j][0][0] * v[i][j][k - 1][0];
        double temp1 = omega * ldz[i][j][0][1] * v[i][j][k - 1][1];
        double temp2 = omega * ldz[i][j][0][2] * v[i][j][k - 1][2];
        double temp3 = omega * ldz[i][j][0][3] * v[i][j][k - 1][3];
        double temp4 = omega * ldz[i][j][0][4] * v[i][j][k - 1][4];
        for (m = 0; m < n; m++) {
          v[i][j][k][m] -= temp0 + temp1 + temp2 + temp3 + temp4;
        }
      }
    }
  }
