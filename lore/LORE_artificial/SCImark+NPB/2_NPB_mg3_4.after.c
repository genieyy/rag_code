...
for (int iter = 0; iter < ITERATIONS; iter++){
	for (i3 = 1; i3 < n3 - 1; i3++)
	{
		for (i2 = 1; i2 < n2 - 1; i2++)
		{
			for (i1 = 0; i1 < n1; i1++)
			{
				double r_i2_m1 = r[i3][i2 - 1][i1];
				double r_i2_p1 = r[i3][i2 + 1][i1];
				double r_i3_m1 = r[i3 - 1][i2][i1];
				double r_i3_p1 = r[i3 + 1][i2][i1];
				
				r1[i1] = r_i2_m1 + r_i2_p1 + r_i3_m1 + r_i3_p1;
				r2[i1] = r[i3 - 1][i2 - 1][i1] + r[i3 - 1][i2 + 1][i1] + r[i3 + 1][i2 - 1][i1] + r[i3 + 1][i2 + 1][i1];
			}
			for (i1 = 1; i1 < n1 - 1; i1++)
			{
				double r_i1_m1 = r[i3][i2][i1 - 1];
				double r_i1_p1 = r[i3][i2][i1 + 1];
				
				u[i3][i2][i1] = u[i3][i2][i1] + c[0] * r[i3][i2][i1] + c[1] * (r_i1_m1 + r_i1_p1 + r1[i1]) + c[2] * (r2[i1] + r1[i1 - 1] + r1[i1 + 1]);
			}
		}
	}
}
