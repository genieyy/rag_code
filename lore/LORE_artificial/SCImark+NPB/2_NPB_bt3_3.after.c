    ...
    for (j = 1; j < n_ - 1; j++) { 
        ...
        for (k = 1; k < m_ - 1; k++) {
            ...
            for (i = 1; i < q_ - 1; i++) {
                ...
                forcing[i][j][k][1] = forcing[i][j][k][1] - tx2 * ((ue[i+1][1] * buf[i+1][1] + c2 * (ue[i+1][4] - q[i+1])) - (ue[i-1][1] * buf[i-1][1] + c2 * (ue[i-1][4] - q[i-1]))) + xxcon1 * (buf[i+1][1] - 2.0 * buf_i_1 + buf[i-1][1]) + dx2tx1 * (ue[i+1][1] - 2.0 * ue_i_1 + ue[i-1][1]);
