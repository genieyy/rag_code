...
for (int iter = 0; iter < ITERATIONS; iter++){
  for (int ti = ist; ti <= iend; ti += 8) {
    for (int tj = jst; tj <= jend; tj += 8) {
      for (i = ti; i < ti + 8 && i <= iend; i++) {
        for (j = tj; j < tj + 8 && j <= jend; j++) {
          for (k = 1; k < nz - 1; k++) {
            for (m = 0; m < n; m++) {
              v[i][j][k][m] -= omega * (ldz[i][j][m][0] * v[i][j][k - 1][0] + ldz[i][j][m][1] * v[i][j][k - 1][1] + ldz[i][j][m][2] * v[i][j][k - 1][2] + ldz[i][j][m][3] * v[i][j][k - 1][3] + ldz[i][j][m][4] * v[i][j][k - 1][4]);
            }
          }
        }
      }
