...
for (int iter = 0; iter < ITERATIONS; iter++) {
    for (i = 0; i < n_; i++) {
        for (k = 0; k < q_; k++) {
            for (m = 0; m < p_; m++) {
                forcing[i][1][k][m] = forcing[i][1][k][m] - dssp * (5.0 * ue[1][m] - 4.0 * ue[1 + 1][m] + ue[1 + 2][m]);
                forcing[i][2][k][m] = forcing[i][2][k][m] - dssp * (-4.0 * ue[2 - 1][m] + 6.0 * ue[2][m] - 4.0 * ue[2 + 1][m] + ue[2 + 2][m]);
            }
            for (m = 0; m < p_; m++) {
                for (j = 1 * 3; j <= m_ - 4; j++) {
            ...
            for (m = 0; m < p_; m++) {
                forcing[i][m_ - 3][k][m] = forcing[i][m_ - 3][k][m] - dssp * (ue[m_ - 3 - 2][m] - 4.0 * ue[m_ - 3 - 1][m] + 6.0 * ue[m_ - 3][m] - 4.0 * ue[m_ - 3 + 1][m]);
                forcing[i][m_ - 2][k][m] = forcing[i][m_ - 2][k][m] - dssp * (ue[m_ - 2 - 2][m] - 4.0 * ue[m_ - 2 - 1][m] + 5.0 * ue[m_ - 2][m]);
            }
        }
    }
}
