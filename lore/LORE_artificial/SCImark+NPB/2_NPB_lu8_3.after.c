	...
	for (i = ist; i <= iend; i++)
	{
		for (j = jst; j <= jend; j++)
		{
			if (nz > 1) {
				...
				double u21k1 = tmp1 * rsd[i][j][1][1];
				double u31k1 = tmp1 * rsd[i][j][1][2];
				double u41k1 = tmp1 * rsd[i][j][1][3];
				double u51k1 = tmp1 * rsd[i][j][1][4];

				double u21km11 = tmp2 * rsd[i][j][0][1];
				double u31km11 = tmp2 * rsd[i][j][0][2];
				double u41km11 = tmp2 * rsd[i][j][0][3];
				double u51km11 = tmp2 * rsd[i][j][0][4];

				flux[i][j][1][1] = tz3 * (u21k1 - u21km11);
				flux[i][j][1][2] = tz3 * (u31k1 - u31km11);
				flux[i][j][1][3] = (4.0 / 3.0) * tz3 * (u41k1 - u41km11);
				flux[i][j][1][4] = 0.50 * (1.0 - C1 * C5) * tz3 * ((u21k1 * u21k1 + u31k1 * u31k1 + u41k1 * u41k1) - (u21km11 * u21km11 + u31km11 * u31km11 + u41km11 * u41km11)) + (1.0 / 6.0) * tz3 * (u41k1 * u41k1 - u41km11 * u41km11) + C1 * C5 * tz3 * (u51k1 - u51km11);
			}
