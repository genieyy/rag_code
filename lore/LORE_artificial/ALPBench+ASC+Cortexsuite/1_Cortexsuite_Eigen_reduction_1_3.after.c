...
#pragma scop
const int TILE_SIZE = 32; // Example tile size
for (int iter = 0; iter < ITERATIONS; iter++) {
    for (int i = 1; i <= n; i += TILE_SIZE) {
        for (int k = 1; k <= m; k += TILE_SIZE) {
            for (int j = 1; j <= m; j += TILE_SIZE) {
                for (int ti = i; ti < i + TILE_SIZE && ti <= n; ti++) {
                    for (int tk = k; tk < k + TILE_SIZE && tk <= m; tk++) {
                        double sum = 0.0;
                        for (int tj = j; tj < j + TILE_SIZE && tj <= m; tj++) {
                            interm[tj] = data[ti][tj];
                            sum += interm[tj] * symmat[tj][m - tk + 1];
                        }
                        data[ti][tk] = sum;
                    }
                }
