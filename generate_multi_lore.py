...
def main(args):
    ...
    ################################################
    def first_prompt(search_code: str, use_rag: bool, results=None, _method_num=args.method_num):
        """
    ...
    please provide {_method_num} best optimized codes. Do not define a new function. The original variable does not need to be redefined. If you generate a new variable, please use the double type.
    Do not output the original C program in your response:
    {search_code}
    there are some examples maybe helpful(if not, just ignore it):
        """
    ...
    def get_final_code_from_llm(search_code: str, feedback_list: list) -> str:
        ...
        if not feedback_list:  #########和no_rag同一个prompt
            prompt = f"""as a compiler, given the C program below,improve its performance using meaning-preserving transformation.
    Do not define a new function. The original variable does not need to be redefined. If you generate a new variable, please use the double type.put your code in the markdown code block.
