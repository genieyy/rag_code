...
for(t = 0; t < _PB_TMAX; t++) {
    ...
    for (i = 1; i < _PB_NX; i++) {
        ey[i][0] -= SCALAR_VAL(0.5) * (hz[i][0] - hz[i-1][0]);
        ex[i][1] -= SCALAR_VAL(0.5) * (hz[i][1] - hz[i][0]);
        hz[i][0] -= SCALAR_VAL(0.7) * (ex[i][1] - ex[i][0] + ey[i+1][0] - ey[i][0]);
        
        for (j = 1; j < _PB_NY - 1; j++) {
        ...
        if (_PB_NY > 1) {
            j = _PB_NY - 1;
            ey[i][j] -= SCALAR_VAL(0.5) * (hz[i][j] - hz[i-1][j]);
            hz[i][j-1] -= SCALAR_VAL(0.7) * (ex[i][j] - ex[i][j-1] + ey[i+1][j-1] - ey[i][j-1]);
        }
    }
}
#pragma endscop
