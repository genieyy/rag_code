...
for (t = 0; t < _PB_TSTEPS; t++) {
  for (int i_tile = 1; i_tile < _PB_N - 1; i_tile += 32) {
    for (int j_tile = 1; j_tile < _PB_N - 1; j_tile += 32) {
      for (i = i_tile; i < min(i_tile + 32, _PB_N - 1); i++) {
        for (j = j_tile; j < min(j_tile + 32, _PB_N - 1); j++) {
