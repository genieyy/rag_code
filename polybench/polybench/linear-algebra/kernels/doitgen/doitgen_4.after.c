...
#pragma scop
for (int rr = 0; rr < _PB_NR; rr += 32)
    for (int qq = 0; qq < _PB_NQ; qq += 32)
        for (int pp = 0; pp < _PB_NP; pp += 32)
            for (r = rr; r < min(rr + 32, _PB_NR); r++)
                for (q = qq; q < min(qq + 32, _PB_NQ); q++)
                    for (p = pp; p < min(pp + 32, _PB_NP); p++) {
