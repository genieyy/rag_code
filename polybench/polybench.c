...
/* Timer code (omp_get_wtime) */
double polybench_o_start, polybench_o_end;
...
#ifdef POLYBENCH_OMP_TIMER
  polybench_o_start = omp_get_wtime();
...
#else
#ifdef POLYBENCH_OMP_TIMER
  printf("%0.6f\n", polybench_o_end - polybench_o_start);
#elif !defined(POLYBENCH_CYCLE_ACCURATE_TIMER)
      printf ("%0.6f\n", polybench_t_end - polybench_t_start);
# else
...
xmalloc(size_t alloc_sz)
{
  ...
  if (! ret || err)
    {
      fprintf (stderr, "[PolyBench] posix_memalign: cannot allocate memory %ld.\n", padded_sz);
      exit (1);
    }
