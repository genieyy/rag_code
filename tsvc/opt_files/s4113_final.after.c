...
for (int nl = 0; nl < iterations; nl++) {
    for (int i = 0; i < LEN_1D; i += 64) { // Tile size of 64
        for (int j = i; j < i + 64 && j < LEN_1D; j += 4) { // Unroll by 4
            a[ip[j]] = b[ip[j]] + c[j];
            a[ip[j + 1]] = b[ip[j + 1]] + c[j + 1];
            a[ip[j + 2]] = b[ip[j + 2]] + c[j + 2];
            a[ip[j + 3]] = b[ip[j + 3]] + c[j + 3];
        }
    }
