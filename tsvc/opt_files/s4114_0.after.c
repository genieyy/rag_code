...
Loop unrolling can reduce the overhead of loop control and improve instruction-level parallelism.
...
for (int nl = 0; nl < iterations; nl++) {
    ...
    for (; i + 3 < LEN_1D; i += 4) {
        ...
        k = ip[i+1];
        a[i+1] = b[i+1] + c[LEN_1D-k+1-2] * d[i+1];
        k += 5;

        k = ip[i+2];
        a[i+2] = b[i+2] + c[LEN_1D-k+1-2] * d[i+2];
        k += 5;

        k = ip[i+3];
        a[i+3] = b[i+3] + c[LEN_1D-k+1-2] * d[i+3];
        k += 5;
    }
    for (; i < LEN_1D; i++) {
        k = ip[i];
