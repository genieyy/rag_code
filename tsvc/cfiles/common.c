...
void save_1d_array(real_t arr[LEN_1D], const char* save_path);

void save_1d_array_half(real_t arr[LEN_1D], const char* save_path);
...
void save_1d_array(real_t arr[LEN_1D], const char* save_path) {
    for (int i = 0; i < LEN_1D; i++) {
        ...
        if (i % 20 == 0) fprintf(TSVC_DUMP_TARGET, "\n");
    }
    fprintf(TSVC_DUMP_TARGET, "\n");
}
...
void save_2d_array(real_t arr[LEN_2D][LEN_2D], const char* save_path) {
    for (int i = 0; i < LEN_2D; i++){
        for (int j = 0; j < LEN_2D; j++){
            ...
            if ((i * LEN_2D + j) % 20 == 0) fprintf (TSVC_DUMP_TARGET, "\n");
        }
    }
    fprintf(TSVC_DUMP_TARGET, "\n");
}
...
void save_array(const char* name) {
    if (!strcmp(name, "s000")) {
        ...
        fprintf(stderr, "Unknown function name passed to dump arrays: %s\n", name);
        exit(1);
    }
}
