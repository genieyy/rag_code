...
if ((L1 <= L2) && (jst <= jend)) {
  for (t1=0;t1<=ITERATIONS-1;t1++) {
    for (t2=L1;t2<=L2;t2++) {
      for (t3=jst;t3<=jend;t3++) {
        for (t4=1;t4<=nz-2;t4++) {
          q = 0.50 * (rsd[t2][t3][t4][1] * rsd[t2][t3][t4][1] + rsd[t2][t3][t4][2] * rsd[t2][t3][t4][2] + rsd[t2][t3][t4][3] * rsd[t2][t3][t4][3]) / rsd[t2][t3][t4][0];;
          u21 = rsd[t2][t3][t4][1] / rsd[t2][t3][t4][0];;
          flux[t2][t3][t4][4] = (C1 * rsd[t2][t3][t4][4] - C2 * q) * u21;;
          flux[t2][t3][t4][3] = rsd[t2][t3][t4][3] * u21;;
          flux[t2][t3][t4][2] = rsd[t2][t3][t4][2] * u21;;
          flux[t2][t3][t4][1] = rsd[t2][t3][t4][1] * u21 + C2 * (rsd[t2][t3][t4][4] - q);;
          flux[t2][t3][t4][0] = rsd[t2][t3][t4][1];;
        }
      }
    }
  }
