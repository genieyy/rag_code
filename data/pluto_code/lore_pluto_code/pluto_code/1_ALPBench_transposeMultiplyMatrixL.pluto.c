...
/* Start of CLooG code */
lbp=0;
ubp=floord(B_col_dim-1,32);
#pragma omp parallel for private(lbv,ubv,t2,t3,t4,t5,t6,t7,t8)
for (t1=lbp;t1<=ubp;t1++) {
  for (t2=0;t2<=floord(A_col_dim-1,32);t2++) {
    for (t3=0;t3<=floord(ITERATIONS-1,32);t3++) {
      for (t4=32*t1;t4<=min(B_col_dim-1,32*t1+31);t4++) {
        for (t5=32*t2;t5<=min(A_col_dim-1,32*t2+31);t5++) {
          for (t6=32*t3;t6<=min(ITERATIONS-1,32*t3+31);t6++) {
            P[t4][t5] = 0;;
            for (t7=0;t7<=A_row_dim-1;t7++) {
              P[t4][t5] += A[t5][t7] * B[t4][t7];;
            }
          }
        }
      }
