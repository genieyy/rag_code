...
for (t1=0;t1<=ITERATIONS-1;t1++) {
  for (t2=0;t2<=nSlip-1;t2++) {
    tauN[t2] = tau[t2];;
    for (t4=0;t4<=nSlip-1;t4++) {
      bor_s_tmp = dtdg[t2][t4] * deltaTime;;
      matrix[t2][t4] = (-bor_s_tmp + dtcdgd[t2][t4]) * bor_array[t2];;
      tauN[t2] += bor_s_tmp * dSlipRate[t4];;
    }
    err[t2] = tauN[t2] - tauc[t2];;
    rhs[t2] = err[t2] * bor_array[t2];;
  }
}
/* End of CLooG code */
