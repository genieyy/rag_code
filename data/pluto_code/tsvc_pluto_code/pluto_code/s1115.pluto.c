...
for (t1=lbp;t1<=ubp;t1++) {
  for (t2=0;t2<=min(min(min(floord(-8*t1+2500024,312495),floord(-64*t1+200*iterations-8,2499935)),floord(-255*t1+312503*LEN_2D-312503,9999841)),floord(-680*t1+2125*iterations+833333*LEN_2D-213333333,26665976));t2++) {
    for (t3=0;t3<=min(min(min(min(floord(25*iterations-1,8),floord(-2500024*t1+7812375*iterations+19687697,2499960)),floord(-19999992*t1+62499975*iterations-2499999,19999480)),floord(-26666656*t1+83333300*iterations+833333*LEN_2D-216666580,26665976)),floord(-80000768*t1+249996025*iterations+2500024*LEN_2D-12499865,79998728));t3++) {
      if ((t2 == 0) && (t3 == 0)) {
        for (t4=32*t1;t4<=min(LEN_2D-1,32*t1+31);t4++) {
          aa[t4][0] = aa[t4][0]*cc[0][t4] + bb[t4][0];;
          ...
          for (t5=1;t5<=31;t5++) {
            for (t6=0;t6<=31;t6++) {
              aa[t4][t5] = aa[t4][t5]*cc[t5][t4] + bb[t4][t5];;
            }
          }
        }
      }
      if ((t2 >= 1) && (t3 == 0)) {
        for (t4=32*t1;t4<=min(LEN_2D-1,32*t1+31);t4++) {
          for (t5=32*t2;t5<=min(LEN_2D-1,32*t2+31);t5++) {
            for (t6=0;t6<=31;t6++) {
              aa[t4][t5] = aa[t4][t5]*cc[t5][t4] + bb[t4][t5];;
            }
          }
