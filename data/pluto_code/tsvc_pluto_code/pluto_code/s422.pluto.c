...
/* Start of CLooG code */
lbp=0;
ubp=floord(iterations-1,4);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
for (t2=lbp;t2<=ubp;t2++) {
  lbv=32*t2;
  ubv=min(8*iterations-1,32*t2+31);
#pragma ivdep
...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5)
for (t2=lbp;t2<=ubp;t2++) {
  for (t3=0;t3<=floord(iterations-1,4);t3++) {
    for (t4=32*t3;t4<=min(8*iterations-1,32*t3+31);t4++) {
      lbv=32*t2;
      ubv=min(LEN_1D-1,32*t2+31);
#pragma ivdep
#pragma vector always
      for (t5=lbv;t5<=ubv;t5++) {
        xx[t5] = flat_2d_array[t5 + 8] + a[t5];;
      }
    }
  }
}
